...
def check_dependencies():
    ...
    try:
        ...
        missing.append("python-telegram-bot requests deep-translator")
    try:
    ...
    if missing:
        ...
        print("pip install python-telegram-bot requests deep-translator yt-dlp")
...
# ================= BYPASS LINK (DIPERBAIKI) =================
def bypass_link(url: str, timeout=15) -> str:
    """Ikuti semua redirect sampai akhir, kembalikan URL final."""
    try:
        # Gunakan allow_redirects=True untuk otomatis mengikuti redirect
...
# ================= FITUR CHAT AI (DIPERBAIKI) =================
async def chat_ai(message: str) -> str:
    """Gunakan API AI gratis yang lebih stabil."""
    try:
        # API 1: vingle.xyz
        url = f"https://api.vingle.xyz/api/ai/chatgpt?text={message}"
        resp = requests.get(url, timeout=30)
        if resp.status_code == 200:
            data = resp.json()
            result = data.get('result') or data.get('response') or data.get('message') or data.get('data')
            if result:
                return result.strip()
        # API 2: popcat.xyz
        url2 = f"https://api.popcat.xyz/chat?msg={message}"
        ...
        # API 3: fallback lainnya
        url3 = f"https://api.lolhuman.xyz/api/openai?apikey=yourkey&text={message}"
        # Tidak digunakan karena butuh API key, tapi sebagai catatan
        return "Maaf, saya tidak bisa menjawab saat ini. Coba lagi nanti."
    except Exception as e:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Pilihan gaya video (untuk nanti)
    ...
    if query.data == 'm_image':
        ...
        await query.edit_message_text(
            "🎨 **Pilih gaya gambar yang diinginkan:**\n\n*Realistis* | *Kartun* | *Anime* | *Fantasi* | *HD* | *Minimalis*",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode=ParseMode.MARKDOWN
        )
        return

    # Tombol pilih gaya video
    ...
    if query.data == 'm_chat':
        ...
        await query.edit_message_text(
            "💬 **MODE AI CHAT**\nKirim pesan apa saja, saya akan menjawab dengan AI.\n\nKetik /start untuk keluar dari mode ini.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Kembali", callback_data='m_back')]])
        )
        return
    ...
    if query.data == 'm_info':
        info_text = (
            ...
            "• 💬 *AI Chat* – ngobrol dengan AI (gratis)\n"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if isinstance(state, dict) and state.get("mode") == "image_style":
        ...
        await status_msg.edit_text(f"🎨 **Sedang melukis...**\n_Prompt: `{final_prompt}`_")
            ...
            if img_path and os.path.exists(img_path):
                await update.message.reply_photo(
                    ...
                    caption=f"✅ **Gambar berhasil dibuat!**\n\n"
                            f"🎨 *Gaya:* {style.capitalize()}\n"
                            f"📝 *Prompt asli:* `{user_input}`\n"
                            f"🌐 *Prompt Inggris:* `{english_prompt}`\n"
                            f"🖌️ *Prompt akhir:* `{final_prompt}`",
                    parse_mode=ParseMode.MARKDOWN
                )
                os.remove(img_path)
            else:
    ...
    # --- MODE VIDEO DENGAN GAYA (nonaktif sementara) ---
    if isinstance(state, dict) and state.get("mode") == "video_style":
        await update.message.reply_text(
            "❌ **Fitur Video AI sedang dalam pengembangan.**\n"
            "Silakan gunakan **Gambar AI**, **AI Chat**, atau **Downloader**."
        )
        USER_STATES[chat_id] = None
        return
    ...
    if isinstance(state, dict) and state.get("mode") == "chat":
        ...
        await progress.start("Menghubungi AI...")
        try:
            ...
            await progress.stop()
            await status_msg.delete()
            await update.message.reply_text("❌ Gagal mendapatkan respons AI. Coba lagi nanti.")
            logging.error(f"Chat AI error: {e}")
        finally:
            ACTIVE_PROCESS[chat_id] = "Idle"
        return
    ...
    if isinstance(state, dict) and state.get("mode") == "bypass":
        if not user_input.startswith(("http://", "https://")):
            await update.message.reply_text("❌ Harap kirimkan link yang valid (diawali http:// atau https://).")
            return
        ...
        try:
            ...
            await progress.stop()
            await status_msg.delete()
            await update.message.reply_text(f"❌ Gagal memproses link: {e}")
        finally:
            ACTIVE_PROCESS[chat_id] = "Idle"
        return

    # Jika tidak dalam mode apapun, arahkan ke menu
    await ls_menu(update, context)
